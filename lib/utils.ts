...
export const getFileType = (fileName: string) => {
  ...
  if (certificationxtensions.includes(extension)) return { type: 'certificados', extension };
...
export const apiList: APIService[] = [
  {
    name: 'Auth Service',
...
export async function pingAPI(
  url: string,
  timeout = 5000,
): Promise<{ online: boolean; time: number | null }> {
  ...
  try {
    ...
    return { online: response.ok, time };
  } catch {
